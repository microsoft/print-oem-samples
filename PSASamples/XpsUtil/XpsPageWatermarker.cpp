...
namespace winrt::XpsUtil::implementation
{
    ...
    HRESULT XpsPageWatermarker::CreateImageResource(_In_ const std::wstring imageFileName, _In_ const std::wstring imagePartName, _Out_ IXpsOMImageResource** imageResource) noexcept try
    {
        ...
        THROW_IF_FAILED(m_xpsFactory->CreateReadOnlyStreamOnFile(imageFileName.c_str(), imageStream.put()));
        THROW_IF_FAILED(m_xpsFactory->CreatePartUri(imagePartName.c_str(), imagePartUri.put()));

        XPS_IMAGE_TYPE imageType = XPS_IMAGE_TYPE_JPEG; // set to type of image being read in
        THROW_IF_FAILED(m_xpsFactory->CreateImageResource(
            imageStream.get(),
            imageType,
            imagePartUri.get(),
            imageResource));
    ...
    HRESULT XpsPageWatermarker::AddImageToVisualCollection(
		...
		float dotsPerInchY, XPS_RECT rectForImage, std::wstring shortDescription) noexcept try
    {
        ...
        THROW_IF_FAILED(m_xpsFactory->CreateImageBrush(imageResource, &viewBox, &viewPort, imageBrush.put()));

        // Create the path that describes the outline of the image on the page.
        THROW_IF_FAILED(CreateRectanglePath(rectForImage, imageRectPath.put()));

        // Set the accessibility description for the path object as required.
        THROW_IF_FAILED(imageRectPath->SetAccessibilityShortDescription(shortDescription.c_str()));

        // Set the image brush to be the fill brush for this path.
        THROW_IF_FAILED(imageRectPath->SetFillBrushLocal(imageBrush.get()));

        // Get the list of visuals for this page...
        THROW_IF_FAILED(xpsPage->GetVisuals(pageVisuals.put()));

        // ...and add the completed path to the list.
        THROW_IF_FAILED(pageVisuals->Append(imageRectPath.get()));

        return S_OK;
    }
    CATCH_RETURN()

    HRESULT XpsPageWatermarker::CreateRectanglePath(_In_ const XPS_RECT& rect, _Out_ IXpsOMPath** rectPath) noexcept try
    {
        ...
        THROW_IF_FAILED(m_xpsFactory->CreateGeometryFigure(&startPoint, rectFigure.put()));
        THROW_IF_FAILED(rectFigure->SetIsClosed(TRUE));
        THROW_IF_FAILED(rectFigure->SetIsFilled(TRUE));
        THROW_IF_FAILED(rectFigure->SetSegments(3, 6, segmentTypes, segmentData, segmentStrokes));

        // Create a geometry that consists of the figure created above.
        THROW_IF_FAILED(m_xpsFactory->CreateGeometry(imageRectGeometry.put()));
        THROW_IF_FAILED(imageRectGeometry->GetFigures(geomFigureCollection.put()));
        THROW_IF_FAILED(geomFigureCollection->Append(rectFigure.get()));
        ...
        THROW_IF_FAILED(m_xpsFactory->CreatePath(reinterpret_cast<IXpsOMPath**>(rectPath)));
        THROW_IF_FAILED((*rectPath)->SetGeometryLocal(imageRectGeometry.get()));
