...
namespace PrintSupportApplication
{
    public sealed partial class PrintTicketManipulationExample : Page
    {
        ...
        private TextBox? CreatePrintTicketFeatureStringInput(PrintTicketParameterInitializer? feature, bool useDefaultEventHandler = true)
        {
            if (feature == null)
            {
                return null;
            ...
            if (useDefaultEventHandler)
            {
                textBox.TextChanged += PrintTicketFeatureStringValueChanged;
            }

            // Set the print ticket feature as the data context. This is used in the SelectionChanged event handler.
            textBox.DataContext = feature;
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            foreach (var feature in printTicketFeatures)
            {
                ...
                if (feature != null)
                {
                    var comboBox = CreatePrintTicketFeatureComboBox(feature);
                    if (comboBox != null)
                    {
                        printTicketControls.Add(comboBox);
                    }
                }
            }
            ...
            var faxPhoneNumber = PrintTicket.GetParameterInitializer("JobFaxPhoneNumber", PRINT_TICKETV11_SCHEMA_NAMESPACE);
            var faxPhoneNumberBox = CreatePrintTicketFeatureStringInput(faxPhoneNumber, false);

            if (faxPhoneNumberBox != null)
            {
                faxPhoneNumberBox.TextChanged += PrintTicketFeatureStringValueChanged;
                printTicketControls.Add(faxPhoneNumberBox);
            }
        ...
        private void PrintTicketFeatureComboBoxSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (feature == null || Session == null || PrintTicket == null) return;
        ...
        private void PrintTicketParameterNumberBoxValueChanged(NumberBox numberBox, NumberBoxValueChangedEventArgs e)
        {
            ...
            if (param == null || Session == null || PrintTicket == null) return;
        ...
        private void PrintTicketFeatureStringValueChanged(object sender, TextChangedEventArgs args)
        {
            var textBox = sender as TextBox;
            if (textBox == null) return;

            var param = textBox.DataContext as PrintTicketParameterInitializer;
            if (param == null || Session == null || PrintTicket == null) return;

            // Disable the ComboBox while saving the print ticket.
            textBox.IsEnabled = false;
            PrintTicket.SetParameterInitializerAsString(param.Name, param.XmlNamespace, textBox.Text);
            Session.UpdatePrintTicket(PrintTicket);
