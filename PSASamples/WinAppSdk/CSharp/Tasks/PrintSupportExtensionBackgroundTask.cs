...
namespace Tasks
{
    public sealed class PrintSupportExtensionBackgroundTask : IBackgroundTask
    {
        ...
        // Track whether the task has been cancelled to avoid work on a torn-down session.
        // In WinAppSDK (in-process), task cancellation and event handlers can race.
        private volatile bool _isCancelled = false;

        // Track active event handlers so we don't complete the task deferral while one is running.
        ...
        private void OnTaskInstanceCanceled(IBackgroundTaskInstance sender, BackgroundTaskCancellationReason reason)
        {
            ...
            // Only complete the deferral if no handlers are currently running.
            // If a handler is active, it will complete the deferral when it finishes.
            if (Interlocked.CompareExchange(ref _activeHandlerCount, 0, 0) == 0)
            {
        ...
        /// <summary>
        /// Helper to safely run event handler work. In WinAppSDK in-process background tasks,
        /// unhandled exceptions propagate into the app process (unlike UWP where they only
        /// crash the separate BackgroundTaskHost.exe). This wrapper catches all exceptions
        /// to prevent 0x3E3 errors from surfacing in the app UI.
        /// </summary>
        private void RunHandlerSafely(System.Action action)
        {
            ...
            {
                // In WinAppSDK in-process, exceptions from print system IPC (e.g., 0x3E3
                // when the session is torn down) must be caught here. In UWP out-of-process,
                // these would only crash the background task host, but in-process they
                // propagate into the app and display as errors in the UI.
            }
            finally
            {
                if (Interlocked.Decrement(ref _activeHandlerCount) == 0 && _isCancelled)
                {
                    // Task was cancelled while this handler was running; complete the deferral now.
                    TaskInstanceDeferral?.Complete();
                }
            }
        }
