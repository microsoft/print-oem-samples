...
namespace PrintSupportApplication
{
    public sealed partial class WatermarkManipulationExample : Page
    {
        ...
        private void RegisterDocumentEventHandlers()
        {
            ...
            document.PageAdded += async (sender, pageNum) =>
            {
                if (pageNum > TotalPages.GetValueOrDefault(0))
                {
                    ...
                    PreviewPaginator.DispatcherQueue.TryEnqueue(() =>
                    {
            ...
            document.XpsGenerationFailed += (sender, e) =>
            {
                XpsGenerationErrorTextBlock.DispatcherQueue.TryEnqueue(() =>
                {
                    PreviewLoadingProgressRing.IsActive = false;
        ...
        /// </summary>
        private void WatermarkPreview_Tapped(object sender, Microsoft.UI.Xaml.Input.TappedRoutedEventArgs e)
        {
